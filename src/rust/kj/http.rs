...
impl<'a, 'b> From<&'b HttpHeaders<'a>> for HttpHeadersRef<'b> {
    fn from(value: &'b HttpHeaders<'a>) -> Self {
        HttpHeadersRef(value.own.as_ref())
    }
}
...
impl<'a> ConnectResponse<'a> {
    ...
    pub fn accept<'h>(
        ...
        ffi::connect_response_accept(self.0, status_code, status_text, headers.into().0);
    }
